...
def create_model_03(input_shape=(299, 299, 3)):
    """
    ...
    # output size (80, 300, 3)
    model.add(Cropping2D(cropping=((55, 30), (10, 10)),
                         input_shape=input_shape, name='crop_80_300'))

    # Image pre-processing
    model.add(Lambda(k_utils.image_preprocess, name='preprocess'))

    # Mean_pool image to 75,75
    model.add(AveragePooling2D(pool_size=(1, 4)))

    # Color information selection by neural net
    model.add(Conv2D(3, 1, 1, subsample=(1,1), activation='sigmoid', name='Conv_01'))

    # Normalize
    #model.add(BatchNormalization(axis=-1, momentum=0.99, epsilon=1e-5))

    model.add(Conv2D(2, 3, 3, subsample=(2,2), activation='elu', name='Conv_02'))
